...
- **Fixed this loop:** #207 defect 1 (comparison operators inverted — commit 29aea6e), #197 defect 1 (arithmetic precedence), #197 defect 3 (if-else-if operators correct), #202 defect 1 (empty loop bodies verified fixed), acc-value ternary detection (commit a5aff78).
...
- **Quality metrics (Kazumi, 3632 methods):** if(undefined)=0 (was 1, now 0), while=78, for=14, do-while=65, if=1247 (was 1557, reduced 20% by ternary detection), switch=4, try=27, lex_=3635, unknown_=30, new acc()=817. Acc-value ternary: 4 `return cond ? a : b` patterns detected. Unknown opcodes (30 total): 15 unknown_FF, 2 unknown_F7, 2 unknown_F1, 1 each of FC/FA/F4/F3/EF/EE/EC/EA/E8/E5/75. All unknown opcodes are instruction alignment issues, NOT real opcodes — ark_disasm produces no unknowns from the same ABC file.
- **Quality metrics (Melotopia, 11788 methods):** new acc()=188 (reduced via register scan), if=1557, while=49, for=15, &&=5, ||=13, lex_=8071.
- **Known-source test HAPs (50 rounds):** Built from `/Users/anakin/DevEcoStudioProjects/MyApplication`. Each round tests specific language patterns. Baselines in `data/test_hap/arkts-decompile-test{1-50}_*.{ets,ts}`.
...
- **Priority:** Fix decompiler quality issues. Vendor opcodes DD-E1 now fully handled. Key remaining: constructor callee recovery for cross-block cases (817 `new acc()` on Kazumi), try-catch body reconstruction, nested ternary chains, 0xFF alignment issues.
