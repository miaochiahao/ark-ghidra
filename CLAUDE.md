...
- **Fixed this loop (continued):** DEFINECLASSWITHBUFFER format fix (commit 5aa99ca), negated comparison simplification (commit b9b63c9), boolean constant folding (commit 6201bc2), if(true) simplification (commit 3dbb6fa), cross-statement single-use variable inlining (commit efaf73f).
...
- **Try-catch body analysis (Kazumi, 456 try blocks):** 266 have empty bodies (`try { }`). Pattern: `try { } catch (e) { actual_code }` — code that should be in the try body appears in the catch handler instead. Likely root cause: try body blocks being consumed by the main CFG loop before `processTryCatch` processes them.
- **New issues:** #209 (page/ability-level output granularity), #210 (undefined register variables v6/v7/v8), #211 (inline string literals — partially addressed by cross-statement inlining).
