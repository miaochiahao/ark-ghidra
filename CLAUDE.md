...
- **Binary operand order (commit 3d435bd):** Ark compiler puts LEFT operand in register, RIGHT operand in accumulator. `InstructionHandler` constructs `BinaryExpression(reg, op, acc)` = `left OP right`. Unit tests use the real compiler convention: lda loads right operand into acc, binary op's register operand is left.
...
- **Null/undefined short-circuit detection (commit ec079df):** `detectShortCircuitPattern` now matches `jequndefined/jeqnull/jstricteqnull/jstrictequndefined` for AND patterns and `jneundefined/jnenull/jnstricteqnull/jnstrictequndefined` for OR patterns. AND conditions with `jeq` null/undefined opcodes require negation (e.g., `jequndefined` returns `x == undefined` but AND needs `x != undefined`). Uses `isAndConditionOpcode`/`isOrConditionOpcode` helpers for maintainability.
