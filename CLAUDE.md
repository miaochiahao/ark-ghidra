...
- **Arithmetic operand order FIX (commit 3b9e2ea, issue #197 defect 1):** Binary ops were using wrong operand order: `BinaryExpression(reg, op, acc)` produced `reg OP acc`. But isa.yaml defines `acc = ecma_op(acc, operand_0)`, so acc is LEFT and reg is RIGHT. Fixed to `BinaryExpression(acc, op, reg)`. Bug was masked for commutative ops (ADD, MUL, EQ) but wrong for SUB, DIV, MOD, LESS, SHL, EXP.
