...
- **Quality metrics (entry-default-unsigned.hap, 5874 methods):** if(undefined)=0, while=186 (was 145), do=127 (was 96), for=113, if=719, lex_0=5242 (increased due to more code exposed by multi-block fix), throw acc=13, func_=368, unknown_=14. Total lines: increased due to previously-truncated branches now being decompiled.
...
- **Priority:** Fix decompiler quality issues. Vendor opcodes DD-E1 now fully handled as callthisNwithname (confirmed via ark_disasm). Key remaining issues: boolean logic patterns (&&/|| produce empty bodies), import resolution, lex variable name resolution.
